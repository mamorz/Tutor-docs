...
public class MyParserSolution {
     ...
     */
    public MyParserSolution(StringReader stringReader) {
    ...
    private Node parseBinaryExpression() throws ParseException {
        Node left = parseAtomicExpression();
        stringReader.readWhile(Character::isWhitespace);

        if (!stringReader.canRead()) {
            throw new ParseException(stringReader, "Expected + or - operator!");
        }

        switch (stringReader.readChar()) {
            case '+':
                stringReader.readWhile(Character::isWhitespace);
                return new Addition(left, parseAtomicExpression());
            case '-':
                stringReader.readWhile(Character::isWhitespace);
                return new Subtraction(left, parseAtomicExpression());
            default:
                throw new ParseException(stringReader, "Expected + or - operators!");
        }
    }

    private Node parseAtomicExpression() throws ParseException {
        if (stringReader.peek(1).equals("(")) {
            stringReader.assertRead("(");
            Node result = parseBinaryExpression();
            stringReader.assertRead(")");
            return result;
        }
    ...
    public static void main(String... args) {
        ...
        while (true) {
            ...
            try {
                Node result = new MyParserSolution(new StringReader(line)).parse();
